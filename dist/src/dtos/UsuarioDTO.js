...
function normalizePerfil(val) {
    ...
    if (typeof val === "string") {
        ...
        if (upper === "SOLICITANTE")
            return Perfil.SOLICITANTE;
        if (upper === "GESTOR")
            return Perfil.GESTOR;
        if (upper === "COMPRADOR")
            return Perfil.COMPRADOR;
    }
    return val;
}
export const perfilSchema = z.preprocess(normalizePerfil, z.enum(Perfil));
const passwordRulesMessage = "Senha: mínimo 6 caracteres, 1 maiúscula, 1 minúscula e 1 caractere especial";
export const createUsuarioSchema = z.object({
    ...
    password: z
        ...
        const missing = [];
        if (!/[A-Z]/.test(s))
            missing.push("1 letra maiúscula");
        if (!/[a-z]/.test(s))
            missing.push("1 letra minúscula");
        if (!/[^A-Za-z0-9]/.test(s))
            missing.push("1 caractere especial");
        if (missing.length) {
            ctx.addIssue({
                code: "custom",
