...
export class ItemPedidoService {
    ...
    async recalcularTotalPedido(em, pedidoId) {
        const itens = await em.find(ItemPedido, { where: { pedido: { id: pedidoId } } });
        const total = itens.reduce((sum, i) => sum + this.num(i.quantidade) * this.num(i.preco_unitario), 0);
    ...
    async create(data) {
        return await this.dataSource.transaction(async (em) => {
            ...
            if (!comRelacoes)
                throw new AppError("Item do pedido não encontrado após criação", 500);
            return comRelacoes;
        });
    }
    async findAll(pedidoId) {
    ...
    async update(id, data) {
        return await this.dataSource.transaction(async (em) => {
            ...
            if (!atualizado)
                throw new AppError("Item do pedido não encontrado após atualização", 500);
            return atualizado;
        });
    }
    async delete(id) {
...
//# sourceMappingURL=ItemPedidoService.js.map